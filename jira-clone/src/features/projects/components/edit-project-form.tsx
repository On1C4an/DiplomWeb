...
export const EditProjectForm = ({ onCancel, initialValues }: EditProjectFormProps) => {
  ...
  const [DeleteDialog, confirmDelete] = useConfirm('Удалить проект', 'Это действие нельзя отменить.', 'destructive');
  ...
  return (
    <div className="flex flex-col gap-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} type="text" placeholder="Введите название проекта" />
                      </FormControl>

                      <FormMessage>{updateProjectForm.formState.errors.name?.message === 'Required' ? 'Обязательно' : updateProjectForm.formState.errors.name?.message}</FormMessage>
                    </FormItem>
                  )}
                />
      ...
      <Card className="size-full border-none shadow-none">
        <CardContent className="p-7">
          <div className="flex flex-col">
            <h3 className="font-bold">Удалить проект</h3>

            <p className="text-sm text-muted-foreground">Удаление проекта необратимо и приведёт к удалению всех связанных данных.</p>
