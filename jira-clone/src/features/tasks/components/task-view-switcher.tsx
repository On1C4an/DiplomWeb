...
import { exportTasksToExcel } from './export-tasks-to-excel';
...
export const TaskViewSwitcher = ({ projectId, hideProjectFilter }: TaskViewSwitcherProps) => {
  ...
  const { data: project } = useGetProject({ projectId: projectId ?? filteredProjectId });
          ...
          <div className="flex gap-2 w-full lg:w-auto">
            {view === 'table' && (
              <Button
                ...
                onClick={() => exportTasksToExcel(tasks?.documents ?? [], project?.name ?? 'Проект')}
                disabled={!tasks?.documents?.length}
              >
                Excel
              </Button>
            )}
